...
from swiftweb.forms import CreateContainerForm, PseudoFolderForm, \
    LoginForm, AddACLForm,CreateObjectForm
from swiftweb.utils import replace_hyphens, prefix_list, \
...
#         print('00')
#         files =request.FILES.getlist('filetmp')
#         for file in files:
#             print(file.name , file.size ,file.read())
    form = CreateObjectForm(request.POST,request.FILES)
#     print('11')
#     print(form)
    if form.is_valid():
        container = form.cleaned_data['containername']
        #handle_uploaded_file(request.FILES['filetmp'])
        filetmps =request.FILES.getlist('filetmp')
        for filetmp in filetmps:
            
        #object = form.cleaned_data['filetmp']
        #print(os.path.basename(object))
            #print(container)
            #print(object)
            try:
                client.put_object(storage_url, auth_token, container,filetmp.name,filetmp.read())
                messages.add_message(request, messages.INFO,
                                         _("Create or replace object."))
            except client.ClientException:
                    messages.add_message(request, messages.ERROR, _("Access denied."))
...
def upload(request, container, prefix=None):
    ...
    signature = hmac.new(b'key', hmac_body.encode('utf-8'), sha1).hexdigest()
...
def download(request, container, objectname):
    ...
    storage_url = request.session.get('storage_url', '')
    auth_token = request.session.get('auth_token', '')
    url = swiftweb.utils.get_temp_url(storage_url, auth_token,
